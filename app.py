...
use_case = {
    "1": "Image background removal - (upload any picture and remove background)",
    "2": "Masking foreground for downstream inpainting task",
}
mask_types = {
...
def get_views(action):
    ...
    if "view_count" not in st.session_state:
        try:
            ...
            data = 0
            print(
                f"Exception in get_views - uncached case. view count not cached: {str(e)}"
            )
        ret_val = data
        ...
        if action != "init":
            try:
                ...
                print(
                    f"Exception in get_views - Non init case. view count not cached: {str(e)}"
                )
    return "{:,}".format(ret_val)
...
def construct_model_info_for_display(model_names, api_info):
    ...
    markdown_str = '<div style="font-size:16px; color: #2f2f2f; text-align: left"><br/><b>Model evaluated </b><br/></div>'
    markdown_str += (
        '<div style="font-size:2px; color: #2f2f2f; text-align: left"><br/></div>'
    )
...
def display_results(results, response_info, mask):
    ...
    st.image(
        results["response"],
        caption=f"Output of Image background removal with mask: {mask}",
    )
...
def app_main(app_mode, example_files, model_name_files, api_info_files, config_file):
    ...
    with open(example_files) as fp:
        _example_file_names = json.load(fp)
    ...
    _curr_use_case = use_case[app_mode].split(".")[0]
    _curr_use_case = use_case[app_mode].split(".")[0]
    st.markdown(
        "<h5 style='text-align: center;'>Image foreground masking or background removal</h5>",
        unsafe_allow_html=True,
    )
    st.markdown(
        f"<div style='color: #4f4f4f; text-align: left'>Image masking using state-of-the-art models for salient object detection(SOD). SOD use cases are<br/>&nbsp;&nbsp;&nbsp;•&nbsp;&nbsp;{use_case['1']}<br/>&nbsp;&nbsp;&nbsp;•&nbsp;&nbsp;{use_case['2']}</div>",
        unsafe_allow_html=True,
    )
    st.markdown(
        f"<div style='color: #9f9f9f; text-align: right'>views:&nbsp;{get_views('init')}</div>",
        unsafe_allow_html=True,
    )
    ...
    try:
        with st.form("twc_form"):
            ...
            selected_file_name = image_select(
                "Select image",
                [
                    "twc_samples/sample1.jpg",
                    "twc_samples/sample2.jpg",
                    "twc_samples/sample3.jpg",
                    "twc_samples/sample4.jpg",
                ],
            )

            st.write("")
            mask_type = st.selectbox(
                label="Select type of masking",
                options=list(dict.keys(mask_types)),
                index=0,
                key="twc_mask_types",
            )
            ...
            _options_arr, markdown_str = construct_model_info_for_display(
                model_names, api_info
            )

            _input_status_area = st.empty()
        ...
        st.download_button(
            ...
            if st.session_state["download_ready"] is not None
            else "",
            disabled=False if st.session_state["download_ready"] is not None else True,
            file_name=(
                st.session_state["model_name"]
                + "_"
                + st.session_state["mask_type"]
                + "_"
                + "_".join(st.session_state["file_name"].split(".")[:-1])
                + ".png"
            ).replace("/", "_"),
            mime="image/png",
            key="download",
